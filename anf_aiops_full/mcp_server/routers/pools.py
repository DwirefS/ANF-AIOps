...
class PoolBody(BaseModel):
    ...
    pool: str
    location: str
    size_tb: int = Field(..., gt=0, description="Capacity in TiB")
...
async def create_pool(body: PoolBody, wait: bool = Query(False)):
    try:
        params = {
            ...
            "size": body.size_tb * 1024 ** 4,
        }
        poller = client().pools.begin_create_or_update(RESOURCE_GROUP, body.account, body.pool, params)
        return poller.result().as_dict() if wait else {"operation": poller.polling_url()}
    except HttpResponseError as err:
        raise sdk_error(err)

@router.patch("/", status_code=202, summary="Resize or retier pool")
async def update_pool(
    ...
    payload = {}
    if new_size_tb:
        payload["size"] = new_size_tb * 1024 ** 4
    if service_level:
        payload["sku"] = {"name": service_level}
    try:
        poller = client().pools.begin_create_or_update(RESOURCE_GROUP, account, pool, payload)
        return poller.result().as_dict() if wait else {"operation": poller.polling_url()}
    except HttpResponseError as err:
        raise sdk_error(err)
